...
async function renderUsersTable() {
  let html = `<table class="users-permissions-table">
    <tr>
      <th>رقم المستخدم</th>
      <th>إدخال كروت<span class="perm-hint">إضافة أكواد</span></th>
      <th>بيع كروت<span class="perm-hint">بيع البطاقات</span></th>
      <th>سجلات العمليات<span class="perm-hint">عرض السجلات كاملة</span></th>
      <th>تحويل رصيد<span class="perm-hint">تحويل رصيد</span></th>
      <th>إدارة الصلاحيات<span class="perm-hint">إدارة باقي المستخدمين</span></th>
      <th>تحديث</th>
    </tr>`;
  allUsers.forEach(u => {
    ...
    html += `<tr>
      ...
      <td><input type="checkbox" ${u.permissions?.addCards ? "checked" : ""} id="addCards-${u.id}"></td>
      <td><input type="checkbox" ${u.permissions?.sellCards ? "checked" : ""} id="sellCards-${u.id}"></td>
      <td><input type="checkbox" ${u.permissions?.seeAllLogs ? "checked" : ""} id="seeAllLogs-${u.id}"></td>
      <td><input type="checkbox" ${u.permissions?.transfer ? "checked" : ""} id="transfer-${u.id}"></td>
      <td><input type="checkbox" ${u.permissions?.managePermissions ? "checked" : ""} id="managePermissions-${u.id}"></td>
      <td><button onclick="updateUserPermissions('${u.id}')">تحديث</button></td>
    </tr>`;
  });
  html += "</table>";
...
window.updateUserPermissions = async function(docid) {
  ...
  const seeAllLogs = document.getElementById(`seeAllLogs-${docid}`).checked;
  const transfer = document.getElementById(`transfer-${docid}`).checked;
  const managePermissions = document.getElementById(`managePermissions-${docid}`).checked;
  await updateDoc(doc(db, "users", docid), {
    permissions: { addCards, sellCards, seeAllLogs, transfer, managePermissions }
  });
...
document.getElementById("permissions-btn").onclick = async () => {
  if (isOwner || userData.permissions?.managePermissions)
    await renderUsersTable(), document.getElementById("permissions-modal").style.display = "flex";
};
